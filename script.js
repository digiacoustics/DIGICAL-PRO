...
loadSavedProjects();
addRoom();
// ... (All the rest of your previous script, unchanged, remains here.)
...
async function downloadPDF() {
  ...
  const calcMain = document.querySelector("main");
  ...
  // ----- COVER PAGE -----
  pdf.setFillColor(84,3,30); // #54031e
  pdf.rect(0, 0, pageWidth, 200, 'F');
  pdf.setTextColor(255,255,255);
  pdf.setFontSize(26);
  pdf.setFont("helvetica", "bold");
  pdf.text("DIGICAL", pageWidth/2, 85, { align: "center" });
  pdf.setFontSize(16);
  pdf.text("Powered by Digi Acoustics", pageWidth/2, 110, { align: "center" });
  pdf.setFontSize(14);
  pdf.text(projectName, pageWidth/2, 145, { align: "center" });
  pdf.setFontSize(10);
  pdf.text(`Date & Time: ${document.getElementById('timestamp').textContent.replace('Date & Time: ', '')}`,
  ...
  // ----- PROJECT REPORT PAGE -----
  pdf.setFontSize(16);
  pdf.setTextColor(84,3,30);
  pdf.setFont("helvetica", "bold");
  y=40;
  pdf.text("Full Project Report", pageWidth/2, y, {align:"center"});
  y += 25;
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(12);
  pdf.text(`Project Name: ${projectName}`, 50, y); y+=20;
  pdf.text(`Client Name: ${document.getElementById('clientName').value || ''}`, 50, y); y+=20;
  pdf.text(`Location: ${document.getElementById('projectLocation').value || ''}`, 50, y); y+=30;

  // For each room
  rooms.forEach((r,i) => {
    const c = calcRoomAreas(r);
    pdf.setFont("helvetica", "bold");
    pdf.setTextColor(44,131,187);
    pdf.text(`Room ${i+1}: ${r.roomName||`Room ${i+1}`}`, 50, y); y+=18;
    pdf.setFont("helvetica", "normal");
    pdf.setTextColor(0,0,0);
    pdf.text(`Wall 1: ${r.length||'0'}ft x ${r.height||'0'}ft = ${c.wall1.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Wall 2: ${r.width||'0'}ft x ${r.height||'0'}ft = ${c.wall2.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Wall 3: ${r.length||'0'}ft x ${r.height||'0'}ft = ${c.wall3.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Wall 4: ${r.width||'0'}ft x ${r.height||'0'}ft = ${c.wall4.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Ceiling Area: ${r.length||'0'}ft x ${r.width||'0'}ft = ${c.ceilArea.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Floor Area: ${r.length||'0'}ft x ${r.width||'0'}ft = ${c.floorArea.toFixed(2)} sq ft`, 50, y); y+=15;
    pdf.text(`Total Wall Area (Gross): ${c.totalWall.toFixed(2)} sq ft`, 50, y); y+=14;
    pdf.text(`Total Deduction Area: ${c.deductionArea.toFixed(2)} sq ft`, 50, y); y+=14;
    pdf.text(`Net Wall Area: ${c.netWall.toFixed(2)} sq ft`, 50, y); y+=14;
    pdf.text(`Acoustic Doors: ${c.doors}`, 50, y); y+=14;
    pdf.text(`Wall Price: ${c.wallPrice && c.netWall>0 ? `₹${(c.netWall*c.wallPrice).toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=13;
    pdf.text(`Ceiling Price: ${c.ceilPrice && c.ceilArea>0 ? `₹${(c.ceilArea*c.ceilPrice).toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=13;
    pdf.text(`Floor Price: ${c.floorPrice && c.floorArea>0 ? `₹${(c.floorArea*c.floorPrice).toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=13;
    pdf.text(`Acoustic Door Price: ${c.doorPrice && c.doors>0 ? `₹${(c.doorPrice*c.doors).toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=13;
    pdf.text(`Room Total: ₹${c.fullCost ? c.fullCost.toLocaleString('en-IN') : 'N/A'}`, 50, y); y+=22;
  ...
  pdf.setFontSize(14);
  pdf.setFont("helvetica", "bold");
  pdf.setTextColor(165,32,50);
  pdf.text("PROJECT SUMMARY", 50, y); y+=18;
  pdf.setTextColor(0,0,0);
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(12);
  pdf.text(`All Rooms Total Net Wall Areas: ${netWall.toFixed(2)} sq ft`, 50, y); y+=15;
  pdf.text(`All Rooms Total Ceiling Areas: ${totalCeil.toFixed(2)} sq ft`, 50, y); y+=15;
  pdf.text(`All Rooms Total Floor Areas: ${totalFloor.toFixed(2)} sq ft`, 50, y); y+=15;
  pdf.text(`All Rooms Total Acoustic Doors: ${totalDoors}`, 50, y); y+=15;
  pdf.text(`Wall Area Price: ${wallCost>0 ? `₹${wallCost.toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=14;
  pdf.text(`Ceiling Area Price: ${ceilCost>0 ? `₹${ceilCost.toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=14;
  pdf.text(`Floor Area Price: ${floorCost>0 ? `₹${floorCost.toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=14;
  pdf.text(`Acoustic Door Price: ${doorCost>0 ? `₹${doorCost.toLocaleString('en-IN')}` : 'N/A'}`, 50, y); y+=14;
  pdf.text(`TOTAL PROJECT COST: ₹${projectCost>0 ? projectCost.toLocaleString('en-IN') : 'N/A'}`, 50, y); y+=18;

  pdf.setTextColor(160,160,160);
  pdf.setFontSize(10);
  pdf.text("Created with DIGICAL by Digi Acoustics", pageWidth/2, y+13, {align:'center'});

  // --- Download
  pdf.save(`${projectName.replace(/\s+/g,'_')}_DIGICAL_Report.pdf`);
}

// ... All other (unchanged) code: tab switching, save/load/delete, share, calculator, etc. follow as in your latest working script ...
